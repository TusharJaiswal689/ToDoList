...
def update_user(user: CurrentUser, db: db_dependency, user_request: UserUpdate):
    updated = us.update_user(db, user_request, user.id)
    if not updated:
        raise HTTPException(status_code=404, detail="User not found.")

@router.put("/me/password", status_code=status.HTTP_204_NO_CONTENT)
def update_password(user: CurrentUser, db: db_dependency, plain_password: UserPassUpdate):
...
def delete_user(user: CurrentUser, db: db_dependency):
